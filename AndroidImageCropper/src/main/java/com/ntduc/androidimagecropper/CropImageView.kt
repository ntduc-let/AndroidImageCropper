...
import com.ntduc.androidimagecropper.BitmapUtils.getRectFromPoints
import com.ntduc.androidimagecropper.BitmapUtils.cropBitmap
import com.ntduc.androidimagecropper.BitmapUtils.cropBitmapObjectHandleOOM
import com.ntduc.androidimagecropper.BitmapUtils.resizeBitmap
import com.ntduc.androidimagecropper.BitmapUtils.rotateBitmapByExif
import com.ntduc.androidimagecropper.BitmapUtils.writeTempStateStoreBitmap
import com.ntduc.androidimagecropper.BitmapUtils.getRectCenterX
import com.ntduc.androidimagecropper.BitmapUtils.getRectCenterY
import com.ntduc.androidimagecropper.BitmapUtils.getRectWidth
import com.ntduc.androidimagecropper.BitmapUtils.getRectHeight
import com.ntduc.androidimagecropper.BitmapUtils.getRectLeft
import com.ntduc.androidimagecropper.BitmapUtils.getRectRight
import com.ntduc.androidimagecropper.BitmapUtils.getRectTop
import com.ntduc.androidimagecropper.BitmapUtils.getRectBottom
...
import com.ntduc.androidimagecropper.CropOverlayView.CropWindowChangeListener
...
class CropImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    var cropShape: CropShape?
        get() = mCropOverlayView!!.mSropShape
        set(cropShape) {
            mCropOverlayView!!.mSropShape = cropShape
        }
    ...
    var guidelines: Guidelines?
        get() = mCropOverlayView!!.mGuidelines
        set(guidelines) {
            mCropOverlayView!!.mGuidelines = guidelines
        }
    ...
    public override fun onSaveInstanceState(): Parcelable? {
        ...
        bundle.putString("CROP_SHAPE", mCropOverlayView.mSropShape?.name)
    ...
    public override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            if (mBitmapLoadingWorkerTask == null && imageUri == null && mBitmap == null && mImageResource == 0) {
                ...
                mCropOverlayView!!.mSropShape = CropShape.valueOf(
    ...
    private fun applyImageMatrix(width: Float, height: Float, center: Boolean, animate: Boolean) {
        if (mBitmap != null && width > 0 && height > 0) {
            ...
            if (center) {
                ...
                if (cropRect != null) {
                    mZoomOffsetX = if (width > getRectWidth(mImagePoints)) 0f else ((width / 2 - cropRect.centerX()).coerceAtMost(
                        -getRectLeft(mImagePoints)
                    ).coerceAtLeast(getWidth() - getRectRight(mImagePoints))
                            / scaleX)
                    mZoomOffsetY = if (height > getRectHeight(mImagePoints)) 0f else ((height / 2 - cropRect.centerY()).coerceAtMost(
                        -getRectTop(mImagePoints)
                    ).coerceAtLeast(getHeight() - getRectBottom(mImagePoints))
                            / scaleY)
                }
    ...
    private fun updateImageBounds(clear: Boolean) {
        if (mBitmap != null && !clear) {
            ...
            // width/height.
            val scaleFactorWidth = 100f * mLoadedSampleSize / getRectWidth(mScaleImagePoints)
            val scaleFactorHeight = 100f * mLoadedSampleSize / getRectHeight(mScaleImagePoints)
 ...
 * Large Bitmaps Efficiently](http://developer.android.com/training/displaying-bitmaps/load-bitmap.html).
    ...
    /** sample size used creating the crop bitmap to lower its size  */ internal constructor(
        /**
    ...
    companion object {
        ...
        private fun getOnMeasureSpec(
            ...
            // Measure Width
            val spec: Int = when (measureSpecMode) {
                MeasureSpec.EXACTLY -> {
                    // Must be this size
                    measureSpecSize
                }
