...
import com.ntduc.androidimagecropper.CropImageView.Guidelines
import com.ntduc.androidimagecropper.CropImageView.CropShape
...
@JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : View(context, attrs) {
    ...
    var mGuidelines: Guidelines? = null
    /** The shape of the cropping area - rectangle/circular.  */
    /** The shape of the cropping area - rectangle/circular.  */
    var mSropShape: CropShape? = null
    ...
    fun setCropShape(cropShape: CropShape) {
        if (this.mSropShape !== cropShape) {
            this.mSropShape = cropShape
            invalidate()
        }
    }
    ...
    private fun drawBackground(canvas: Canvas) {
        ...
        val left = Math.max(getRectLeft(mBoundsPoints), 0f)
        val top = Math.max(getRectTop(mBoundsPoints), 0f)
        val right = Math.min(getRectRight(mBoundsPoints), width.toFloat())
        val bottom = Math.min(getRectBottom(mBoundsPoints), height.toFloat())
        if (mSropShape === CropShape.RECTANGLE) {
    ...
    private fun drawCorners(canvas: Canvas) {
        if (mBorderCornerPaint != null) {
            ...
            val w: Float = (cornerWidth / 2
                    + if (mSropShape === CropShape.RECTANGLE) mBorderCornerOffset else 0f)
            val rect = mCropWindowHandler.rect
            rect!!.inset(w, w)
    ...
    private fun onActionDown(x: Float, y: Float) {
        mMoveHandler = mCropWindowHandler.getMoveHandler(x, y, mTouchRadius, mSropShape)
