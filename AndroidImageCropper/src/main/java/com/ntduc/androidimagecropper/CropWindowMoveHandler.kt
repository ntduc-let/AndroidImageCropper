...
internal class CropWindowMoveHandler(
        ...
        if (aspectRatio > 0) {
            ...
            if (topMoves && bottomMoves) {
                ...
                if (bottomMoves && rect.top + newHeight > bounds.bottom) {
                    newLeft =
                        newLeft.coerceAtLeast(bounds.left.coerceAtLeast(rect.right - (bounds.bottom - rect.top) * aspectRatio))
                }
            }
        }
        rect.left = newLeft
    ...
    private fun adjustRight(
        ...
        if (aspectRatio > 0) {
            ...
            // Checks if the window is too small vertically
            if (newHeight < mMinCropHeight) {
                newRight = bounds.right.coerceAtMost(rect.left + mMinCropHeight * aspectRatio)
                newHeight = (newRight - rect.left) / aspectRatio
            }

            // Checks if the window is too large vertically
            if (newHeight > mMaxCropHeight) {
                newRight = bounds.right.coerceAtMost(rect.left + mMaxCropHeight * aspectRatio)
                newHeight = (newRight - rect.left) / aspectRatio
            }
            ...
            if (topMoves && bottomMoves) {
                newRight =
                    newRight.coerceAtMost(bounds.right.coerceAtMost(rect.left + bounds.height() * aspectRatio))
            } else {
                // if top edge moves by aspect ratio check that it is within bounds
                if (topMoves && rect.bottom - newHeight < bounds.top) {
                    newRight =
                ...
                if (bottomMoves && rect.top + newHeight > bounds.bottom) {
                    newRight =
                        newRight.coerceAtMost(bounds.right.coerceAtMost(rect.left + (bounds.bottom - rect.top) * aspectRatio))
                }
            }
        }
        rect.right = newRight
        ...
        if (aspectRatio > 0) {
            ...
            if (leftMoves && rightMoves) {
                ...
                if (rightMoves && rect.left + newWidth > bounds.right) {
                    newTop =
                        newTop.coerceAtLeast(bounds.top.coerceAtLeast(rect.bottom - (bounds.right - rect.left) / aspectRatio))
                }
            }
        }
        rect.top = newTop
    ...
    private fun adjustBottom(
        ...
        if (aspectRatio > 0) {
            ...
            // Checks if the window is too small horizontally
            if (newWidth < mMinCropWidth) {
                newBottom = bounds.bottom.coerceAtMost(rect.top + mMinCropWidth / aspectRatio)
                newWidth = (newBottom - rect.top) * aspectRatio
            }

            // Checks if the window is too large horizontally
            if (newWidth > mMaxCropWidth) {
                newBottom = bounds.bottom.coerceAtMost(rect.top + mMaxCropWidth / aspectRatio)
                newWidth = (newBottom - rect.top) * aspectRatio
            }
            ...
            if (leftMoves && rightMoves) {
                newBottom =
                    newBottom.coerceAtMost(bounds.bottom.coerceAtMost(rect.top + bounds.width() / aspectRatio))
            } else {
                // if left edge moves by aspect ratio check that it is within bounds
                if (leftMoves && rect.right - newWidth < bounds.left) {
                    newBottom =
                ...
                if (rightMoves && rect.left + newWidth > bounds.right) {
                    newBottom =
                        newBottom.coerceAtMost(bounds.bottom.coerceAtMost(rect.top + (bounds.right - rect.left) / aspectRatio))
                }
            }
        }
